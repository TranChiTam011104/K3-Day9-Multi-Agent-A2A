"""
Data Loader Module
Load v├á cache c├íc CSV files tß╗½ Olist dataset.
"""
...
# Global cache cho c├íc DataFrames
_cache: Dict[str, pd.DataFrame] = {}


def load_csv(filename: str, use_cache: bool = True) -> pd.DataFrame:
    """
    Load mß╗Öt CSV file v├áo DataFrame.

    Args:
        filename: T├¬n file trong th╞░ mß╗Ñc data/
        use_cache: C├│ d├╣ng cache kh├┤ng

    Returns:
        DataFrame chß╗⌐a dß╗» liß╗çu
    """
...
def load_all_csvs() -> Dict[str, pd.DataFrame]:
    """
    Load tß║Ñt cß║ú CSV files cß║ºn thiß║┐t.

    Returns:
        Dict vß╗¢i key = filename, value = DataFrame
    """
    files = [
...
def get_order_with_details(order_id: str) -> Dict[str, Any]:
    """
    Lß║Ñy th├┤ng tin chi tiß║┐t cß╗ºa mß╗Öt order.

    Args:
        order_id: Order ID cß║ºn tra cß╗⌐u

    Returns:
        Dict chß╗⌐a order info, items, payments, v.v.
    """
...
def analyze_delivery_timing(order_id: str) -> Dict[str, Any]:
    """
    Ph├ón t├¡ch thß╗¥i gian giao h├áng.

    Args:
        order_id: Order ID cß║ºn ph├ón t├¡ch

    Returns:
        Dict chß╗⌐a delivery timing info
    """
...
def check_payment_reconciliation(order_id: str, tolerance: float = 0.10) -> Dict[str, Any]:
    """
    Kiß╗âm tra payment c├│ khß╗¢p vß╗¢i item + freight kh├┤ng.

    Args:
        order_id: Order ID cß║ºn kiß╗âm tra
        tolerance: Sai sß╗æ cho ph├⌐p (default 0.10 BRL)

    Returns:
        Dict chß╗⌐a payment reconciliation info
    """
    order_data = get_order_with_details(order_id)
    if "error" in order_data:
        return order_data
...
def list_input_cases() -> list:
    """
    Liệt kê tất cả input case IDs.

    Returns:
        List của case IDs (e.g., ["EC_001", "EC_002", ...])
    """
    from pathlib import Path
    input_dir = Path(__file__).parent.parent / "input"
    case_ids = sorted([f.stem for f in input_dir.glob("EC_*.json")])
    return case_ids
